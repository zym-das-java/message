        ...
        $(function () {
            ...
            $.ajax({
                ...
                success : function (data) {
                    ...
                    var breed = "<option value=''>请选择</option>";
                    for (var i = 0; i <data.length ; i++) {
                        breed += '<option value="'+data[i].id+'">'+data[i].cropSpecies+'</option>'
                    }

                    $("[name=breed]").html(breed)
    ...
    <div class="im-content">
        <div class="im-heade">
            ...
            <div class="imhead-select">
                <form name="form" method="post" action="/find/All">
                <ul>
                    <li class="bh">
                        <lable>样品编号</lable>
                    ...
                    <li class="w mright yu">
                        <lable>农作物种类</lable>
                        <select name="breed" class="nzw_spacies" id="crop_species">
                            <option value="">请选择</option>
                        </select>
                    </li>
                    <li class="place w">
                ...
                <div class="soso"><input type="submit" value="搜索"></div>
                </form>
            </div>
        ...
        <div class="im-table">
            <table>
                ...
                <tbody>
                <c:forEach items="${list.result}" var="sam">
                    <tr>
                        <td><input class="testyangpin" value="${sam.id}" type="checkbox"></td>
                        <td class="ybbh">${sam.sampleId}</td>
                        <td>${sam.addressCity.name}-${sam.addressProvince.name}-${sam.addressTown.name}</td>
                        <td>${sam.cropSpecies.cropSpecies}</td>
                        <td><fmt:formatDate value="${sam.harvestTime}" pattern="yyyy-MM-dd"></fmt:formatDate> </td>
                        <td><fmt:formatDate value="${sam.samplingTime}" pattern="yyyy-MM-dd"></fmt:formatDate></td>
                        <td>${sam.pollutionRate}</td>
                        <td>${sam.sampleToxinInfo.toxinType}</td>
...
<div class="im-page">
    <ul>
        <li class="im-indexpage"><a href="/find/All?pageNum=1">首页</a></li>
        <li class="im-nextpage">
            <a <c:if test="${list.pageNum != list.nextPage}">href="/find/All?pageNum=${list.prePage}"</c:if>
                    <c:if test="${list.pageNum == list.nextPage}">  class="disabled"</c:if>>上一页</a></li>
        <li class="im-nextpage">
            <a <c:if test="${list.pageNum == list.finalPage}">class="disabled" </c:if>
               <c:if test="${list.pageNum != list.finalPage}">href="/find/All?pageNum=${list.nextPage }"</c:if>>下一页</a></li>
        <li class="im-indexpage"><a href="/find/All?pageNum=${list.pages}">尾页</a></li>
    </ul>
</div>
